/**
 * StackBid — Location Pages Generator (Programmatic SEO)
 *
 * Ідея з ai-Ковчег "Навигатор скиллов" (programmatic-seo skill, 09.09).
 * "Locations" playbook: "[service] cost in [city]" pages, targeting the
 * 5 launch regions established back in June (NJ, MD, NC — Charlotte/
 * Raleigh, TX — Dallas/Houston, FL — Orlando/Tampa).
 *
 * КРИТИЧНО (сам скіл про це прямо попереджає): це НЕ шаблон з підміненою
 * назвою міста — кожна сторінка генерується окремим web-search-дослідженням
 * (як seo-agent.js), з реальними регіональними відмінностями (клімат
 * впливає на тип покрівлі, вартість праці різна по регіону, дозвільні
 * вимоги різні). Той самий анти-фабрикаційний контур, що і в seo-agent.js:
 * жодних вигаданих цифр/джерел, тільки те, що реально знайшлося в пошуку.
 *
 * Одноразовий скрипт (не крон) — запускається вручну по мірі додавання
 * нових комбінацій city×type, не за розкладом.
 *
...
// Той самий системний промпт, що і в seo-agent.js по духу — web-search
// дослідження, заборона на вигадані цифри/джерела, чіткий формат відповіді.
    ...
    for (const city of CITIES) {
      ...
      await supabaseFetch('location_pages', {
        ...
        body: JSON.stringify({
          ...
          faq_json: page.faq,
          status: 'draft', // Игорь проверяет перед публикацией, тот же порядок, что у /blog
        }),
      });
      created += 1;
      console.log(
